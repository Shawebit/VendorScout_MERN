...
const VendorProfile = () => {
  ...
  const [imageUrls, setImageUrls] = useState(['', '', '', '', '']); // Max 5 images
  ...
  const fetchVendorProfile = async () => {
    try {
      ...
      // Initialize image URLs from existing images
      if (response.data.images && response.data.images.length > 0) {
        const urls = response.data.images.map(img => img.url);
        // Pad with empty strings to ensure 5 slots
        while (urls.length < 5) {
          urls.push('');
        }
        setImageUrls(urls);
  ...
  const handleSave = async () => {
    try {
      ...
      dataToSend.images = validImageUrls.map(url => ({ url }));
            ...
            <div className="vendor-dashboard-card">
              ...
              {isEditing ? (
                <div>
                  ...
                  <div style={{ marginBottom: '1rem' }}>
                    <label style={{ display: 'block', marginBottom: '0.25rem', fontWeight: '500' }}>
                      Vendor Images (Max 5) - Enter Image URLs
                    </label>
                    <p style={{ fontSize: '0.875rem', color: '#666', marginBottom: '0.5rem' }}>
                      Enter direct URLs to your images (must start with http:// or https://)
                    </p>
                    {[0, 1, 2, 3, 4].map((index) => (
                  ...
                  {/* Preview of images when editing */}
                  <div>
                    <h4 style={{ color: 'var(--color-medium-grey)', fontSize: 'var(--font-size-xs)', marginBottom: '0.25rem', fontWeight: '400' }}>Image Preview</h4>
                    {profileData.images && profileData.images.length > 0 ? (
                      ...
                      <p style={{ color: 'var(--color-medium-grey)', fontSize: 'var(--font-size-xs)', fontStyle: 'italic' }}>
                        No images added yet.
                      </p>
                    )}
                  </div>
                ...
                <div>
                  ...
                  <div>
                    <h4 style={{ color: 'var(--color-dark-grey)', fontSize: 'var(--font-size-base)', marginBottom: '0.5rem', fontWeight: '600' }}>Gallery</h4>
                    {profileData.images && profileData.images.length > 0 ? (
