...
const VendorDetail = () => {
  ...
  const [commentLikes, setCommentLikes] = useState({}); // Track like status for each comment
  ...
  const fetchVendorComments = async (vendorId) => {
    try {
      ...
      // Initialize like status for each comment
      const likes = {};
      response.data.forEach(comment => {
        likes[comment._id] = comment.likedBy?.includes(user?.id) || false;
      });
  ...
  const handleSubmitComment = async () => {
    ...
    if (commentText.length > 280) {
      showToast('Comment must be 280 characters or less.', 'warning');
      return;
    }
  ...
  if (loading) {
    return (
      <div className="customer-dashboard">
        ...
        <div className="customer-dashboard-container" style={{ textAlign: 'center', padding: '4rem 2rem' }}>
          <p style={{ fontSize: '0.875rem', color: '#666666', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Loading vendor details...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="customer-dashboard">
        ...
        <nav className="customer-navbar">
          <div className="customer-navbar-container">
            <div className="navbar-left">
              <div className="pincode-search-container">
                <input
                  ...
                  placeholder="Enter pincode"
                  maxLength="6"
                  className="vendor-detail-pincode-input"
                />
                <button 
                  onClick={handlePincodeSearch}
                  className="vendor-detail-pincode-btn"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                    <path d="M8 16s6-5.686 6-10A6 6 0 0 0 2 6c0 4.314 6 10 6 10zm0-7a3 3 0 1 1 0-6 3 3 0 0 1 0 6z"/>
                  </svg>
            ...
            <div className="vendor-detail-nav-center">
              <h1 className="vendor-detail-logo" onClick={() => navigate('/customer/dashboard')}>
                VendorScout
              </h1>
            </div>
            
            <div className="vendor-detail-nav-right">
              <button onClick={() => navigate('/customer/map')} className="vendor-detail-nav-btn">
                Live Map
              </button>
              <button onClick={() => navigate('/customer/discussion')} className="vendor-detail-nav-btn">
                Community
              </button>
              <button onClick={() => navigate('/customer/followed')} className="vendor-detail-nav-btn">
                Followed
              </button>
              <button onClick={handleLogout} className="vendor-detail-nav-btn">
                Logout
              </button>
            </div>
          </div>
  ...
  if (!vendor) {
    return (
      <div className="vendor-detail-page">
        {/* Navigation Bar */}
        <nav className="vendor-detail-nav">
          <div className="vendor-detail-nav-container">
            <div className="vendor-detail-nav-left">
              <div className="vendor-detail-pincode-container">
                <input
                  type="text"
                  value={pincodeInput}
                  onChange={(e) => setPincodeInput(e.target.value)}
        ...
        <div className="customer-dashboard-container" style={{ textAlign: 'center', padding: '4rem 2rem' }}>
          <p style={{ fontSize: '0.875rem', color: '#666666', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Vendor not found</p>
        </div>
      </div>
    );
  }
    ...
    <div className="customer-dashboard">
      ...
      <div className="customer-dashboard-container" style={{ maxWidth: '1400px', margin: '0 auto', padding: '3rem 2rem' }}>
        ...
        <div style={{ marginBottom: '2rem' }}>
          <button onClick={() => navigate('/customer/dashboard')} style={{ background: 'none', border: '1px solid #000000', padding: '0.75rem 1.5rem', cursor: 'pointer', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.1em', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              {vendorMenuItems.map((item) => (
                <div key={item._id} style={{ border: '1px solid #000000', overflow: 'hidden', backgroundColor: item.isAvailable ? '#ffffff' : '#f5f5f5' }}>
                  ...
                  <div style={{ padding: '1.5rem', position: 'relative' }}>
                    ...
                    <div style={{ fontSize: '1rem', fontWeight: '600', color: '#000000' }}>₹{item.price}</div>
                  </div>
                </div>
              ))}
            </div>
                ...
                <div key={comment._id} style={{ border: '1px solid #e5e5e5', padding: '1.5rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', paddingBottom: '0.75rem', borderBottom: '1px solid #f5f5f5' }}>
                    ...
                    <button
                      ...
                      style={{ background: 'none', border: '1px solid #e5e5e5', padding: '0.5rem 1rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.75rem', color: commentLikes[comment._id] ? '#000000' : '#666666' }}
                    >
                      ♥ {comment.likes || 0}
                    </button>
                  </div>
